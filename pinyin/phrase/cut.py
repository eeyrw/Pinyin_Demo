...
def calc(sentence, DAG, route):
    ...
    for idx in range(N - 1, -1, -1):
        route[idx] = max((FREQ.get(' '.join(sentence[idx:x + 1])) + route[x + 1][0], x) for x in DAG[idx])
...
def get_max_key(m):
    result = sorted(list(m.items()), key=lambda d: d[1], reverse=True)
    return result[0][0]
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        for x in cut_DAG_NO_HMM(pinyin_list):
            phrase_map = PhrasePinyin.get_by_pinyin(' '.join(x))
            if phrase_map:
